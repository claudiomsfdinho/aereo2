...
public class VooAdapter  extends BaseAdapter implements SectionIndexer
{
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(view == null) {
            ...
            ImageView imagemV = (ImageView)view.findViewById(R.id.imagemVImageView);
            ...
            view.setTag(new ViewHolder(imagemV, nomeVoo, detalhesVoo));
        }
        ...
        holder.getImagemV().setImageDrawable(drawable);
        Locale locale = new Locale("pt", "BR");
        NumberFormat formatter = NumberFormat.getCurrencyInstance(locale);
        holder.getNomeVoo().setText(voos[position].getNome());
        holder.getDetalhesVoo().setText(String.format("%s - %s", voos[position].getQtdeEscalas(),
                formatter.format(voos[position].getValor())));
